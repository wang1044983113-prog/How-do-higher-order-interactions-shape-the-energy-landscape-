...
max_sim_time = 1500; % 最大模拟时间（计算离出时间）
dt = 0.1;           % 时间步长
prep_steps = prep_time/dt;
max_steps = max_sim_time/dt;

% 新离出条件参数
no_noise_time = 5;    % 去掉噪声后计算的时间（改为5秒）
...
for sd_idx = 1:num_sigma_delta
    ...
    parfor sample_idx = 1:num_samples
        ...
        % 无噪声系统演化，确保状态稳定
        theta_final = stabilize_initial_state(initial_phases, n, r, sigma, sigma_delta, prep_steps, dt);
    ...
    % 计算Twisted到Non-twisted的离出时间
    fprintf('\n第二阶段：计算Twisted到Non-twisted的离出时间 (噪声强度 %.3f-%.3f):\n', min(D_values), max(D_values));
    fprintf('优化：每 %.1f 秒检测一次状态转变 (每 %d 个时间步)\n', detect_interval, detect_steps);
    fprintf('新离出条件：每个时间点去掉噪声，计算 %d 秒后的P_order，小于 %.2f 算离出\n', no_noise_time, exit_threshold);
    ...
    for d_idx = 1:length(D_values)
        ...
        % 输出当前进度和统计信息
        if valid_samples(d_idx) > 0
            fprintf('  Twisted到Non-twisted: %d/%d (有效样本/总样本, %.1f%%)\n', ...
                valid_samples(d_idx), total_samples, ...
                100*valid_ratio(d_idx));
            fprintf('  平均离出时间 MFPT = %.2f\n', mean_exit_time(d_idx));
        else
            fprintf('  Twisted到Non-twisted: %d/%d (有效样本/总样本, %.1f%%), 平均离出时间 NaN (无有效样本)\n', ...
                valid_samples(d_idx), total_samples, ...
                100*valid_ratio(d_idx));
        end
...
marker_sizes = [14, 14, 14, 14, 14, 14];  % 标记大小改为14
...
% 设置图表标题
title('ln(\tau_e) vs. 1/D^2 with Mixed Method: Mean-then-Log for Data, Log-then-Mean for Error (Twisted to Non-twisted)', ...
...
for sd_idx = 1:num_sigma_delta
    legend_str{sd_idx} = sprintf('\\sigma_{\\Delta} = %.1f (\\Delta V \\approx %.4f)', ...
...
% save('mean_exit_time_results_twisted_to_nontwisted_mixed_method.mat', ...
...
% 
% % 保存图片
% saveas(fig1, 'ln_tau_vs_1_D2_twisted_to_nontwisted_mixed_method.png');
