...
% 新离出条件：每个时间点去掉噪声，算50s，计算P_order，大于0.9算离出
% 添加双坐标轴误差棒显示 - 使用与finalMFPTlow相同的误差计算方式
...
% 检测间隔设置 - 优化检测频率
detect_interval = 1;  % 每2秒检测一次状态，而不是每一步
detect_steps = round(detect_interval/dt);  % 对应的步数间隔

% 初始non-twisted阈值（仍然使用固定值来生成初始样本）
initial_nontwisted_threshold = 0.6;  % P_order < 0.6 为初始non-twisted状态

% 设置噪声强度范围 - 使用均匀分布的1/D²值重新计算D_values
% 调整为更合理的范围
inv_D_squared_min = 1/(0.6^2);  % 对应于D=0.6的1/D²
inv_D_squared_max = 1/(0.4^2);  % 对应于D=0.4的1/D²
...
for sd_idx = 1:num_sigma_delta
    ...
    % 注意：all_exit_times_data已经在内层循环中保存了
end
...
for sd_idx = 1:num_sigma_delta
    for i = 1:length(D_values)
        if valid_samples_all(sd_idx, i) > 0 && ~isempty(all_exit_times_data{sd_idx, i})
            ...
            % 然后计算对数数据的均值和标准差
            y_new_all(sd_idx, i) = mean(log_exit_times);
            y_err_all(sd_idx, i) = std(log_exit_times);
        else
            y_new_all(sd_idx, i) = NaN;
...
% 创建适中的背景颜色（用于柱状图）
light_colors = colors + 0.35 * (1 - colors);  % 向白色方向混合

% 设置线型和标记类型
marker_styles = {'+', 'x', 'v'}; % 加号、叉号、倒三角
marker_sizes = [12, 12, 12]; % 增大标记尺寸，使其更明显
...
% 设置图表标题
title('ln(\tau_e) and Standard Deviation vs. 1/D^2 for Non-twisted to Twisted Transition (High \sigma_\Delta)', ...
...
% 注意：在双坐标轴图中，左右轴的图形对象层次关系已经通过绘制顺序确定
% 左坐标轴的元素会自动在右坐标轴元素之上

% % 保存所有结果数据（包括原始数据和误差信息）
% save('mean_exit_time_results_high_sigma_with_errorbars.mat', ...
...
% 
% % 保存图片
% saveas(fig1, 'ln_tau_and_std_vs_1_D2_high_sigma_delta.png');
...
fprintf('\n新离出条件: 每个时间点去掉噪声，计算 %d 秒后的P_order > %.2f\n', no_noise_time, exit_threshold);
fprintf('使用方法2计算误差棒: 先对每个数据点取对数，再计算统计量（与finalMFPTlow相同）\n');
